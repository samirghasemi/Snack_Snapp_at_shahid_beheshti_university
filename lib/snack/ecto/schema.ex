defmodule Arango.Ecto.Repo.Schema do
  ...
  def prepare_changeset_keep_virtual(repo, %Changeset{valid?: true} = changeset, opts) do
  ...
  def prepare_changeset_keep_virtual(repo, %Changeset{valid?: false} = changeset, opts) do
    {:error, put_repo_and_action(changeset, :insert, repo, opts)}
  end


  def prepare_changeset_for_insert(repo, %Changeset{valid?: true} = changeset, opts) do
    ...
    schema_meta = metadata(struct_, autogen_id, opts)
  ...
  def prepare_changeset_for_insert(repo, %Changeset{valid?: false} = changeset, opts) do
    {:error, put_repo_and_action(changeset, :insert, repo, opts)}
  end
      ...
      case Arangox.post(
            ...
            document
             ...
             {:ok, _, %{body: body}} ->
               ...
               values = Map.from_struct(to_struct(struct_, body["new"]))
               values = Map.merge(Map.new(extra), values)
  ...
  defp do_update(repo, name, %Changeset{valid?: true} = changeset, opts) do
    ...
    force? = !!opts[:force]
    # filters = add_pk_filter!(changeset.filters, struct_)
  ...
  defp do_delete(repo, name, %Changeset{valid?: true} = changeset, opts) do
    ...
    # wrap_in_transaction(adapter, adapter_meta, opts, assocs != [], prepare, fn ->
      ...
      if changeset.valid? do
        ...
        if changeset.valid? do
          case Arangox.delete(name, "/_api/document/#{collection(changeset)}/#{key}") do
            a = {:ok, _, %{body: body}} ->
              changeset = load_changes(changeset, :deleted, [], %{}, %{}, [], adapter, schema_meta)
  ...
  def load(adapter, schema_or_types, data) do
    do_load(schema_or_types, data, &Arango.Ecto.Type.adapter_load(adapter, &1, &2))
  end
  ...
  defp autogenerate_id({key, source, type}, changes, return_types, return_sources, adapter) do
    cond do
      Map.has_key?(changes, key) -> # Set by user
        {changes, [], return_types, return_sources}
      value = Arango.Ecto.Type.adapter_autogenerate(adapter, type) -> # Autogenerated now
