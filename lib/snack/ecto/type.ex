defmodule Arango.Ecto.Type.Custom do
  defmacro __using__(opts) do
    quote do
      @maptypes unquote(Keyword.get(opts, :maptypes))
      @keys Map.keys(@maptypes)
      @map_list Map.to_list(@maptypes)

      defstruct @keys

      use Arango.Ecto.Type
      def type, do: @map_list
      ...
      def cast(%{} = attrs) do
        case {%{}, @maptypes}
          |> Ecto.Changeset.cast(attrs, @keys)
        do
          %{valid?: true, changes: changes} ->
            {:ok, changes}
          %{valid?: false, errors: custom_errors} ->
             {:error, custom_errors}
        end
      end
      def cast(_) do
      ...
      # def validate_required(changeset, keys) do
      #   changeset
      ...
      defp load_field!(field, type, value, adapter) do
        case Arango.Ecto.Type.adapter_load(adapter, type, value) do
          {:ok, value} ->
            value
          :error ->
            raise Ecto.ChangeError,
                  "value `#{inspect(value)}` for `#{}.#{field}` " <>
                  "in `#{}` does not match type #{inspect type}"
        end
      end
      defp load_fields!(kw, loader, adapter) do
        for {field, value} <- kw do
          type = Map.fetch!(loader, field)
          {field, load_field!(field, type, value, adapter)}
        end
      end

      def load(%{} = data) do
        {:ok, struct(__MODULE__, Map.new load_fields!(data, @maptypes, Arango.Ecto.Adapter))}
      ...
      defp dump_field!(field, type, value, adapter) do
      ...
      defp dump_fields!(kw, dumper, adapter) do
        for {field, value} <- kw do
          type = Map.fetch!(dumper, field)
          {field, dump_field!(field, type, value, adapter)}
        end
      end
      def dump(struct) do
        {:ok, Map.new dump_fields!(struct, @maptypes, Arango.Ecto.Adapter)}
      end
      def dump(), do: :error
    end
  end
...
defmodule Arango.Ecto.Type do
  ...
  def dump(:date, value, _dumper), do: dump_date(value)
  ...
  defp dump_date(%Date{} = term), do: {:ok, Date.to_iso8601(term)}
  defp dump_date(_), do: :error

  defp dump_time(%Time{} = term), do: {:ok, Time.to_iso8601 check_no_usec!(term, :time)}
  defp dump_time(_), do: :error

  defp dump_time_usec(%Time{} = term), do: {:ok, Time.to_iso8601 check_usec!(term, :time_usec)}
  defp dump_time_usec(_), do: :error

  defp dump_any_datetime(%NaiveDateTime{} = term), do: {:ok, NaiveDateTime.to_iso8601 term}
  defp dump_any_datetime(%DateTime{} = term), do: {:ok, DateTime.to_iso8601 term}
  defp dump_any_datetime(_), do: :error

  defp dump_naive_datetime(%NaiveDateTime{} = term), do:
    {:ok, NaiveDateTime.to_iso8601 check_no_usec!(term, :naive_datetime)}

  defp dump_naive_datetime(_), do: :error

  defp dump_naive_datetime_usec(%NaiveDateTime{} = term),
    do: {:ok, NaiveDateTime.to_iso8601 check_usec!(term, :naive_datetime_usec)}

  defp dump_naive_datetime_usec(_), do: :error

  defp dump_utc_datetime(%DateTime{} = datetime) do
    kind = :utc_datetime
    {:ok, datetime |> check_utc_timezone!(kind) |> check_no_usec!(kind) |> DateTime.to_iso8601}
  end
  ...
  defp dump_utc_datetime_usec(%DateTime{} = datetime) do
    ...
    {:ok, datetime |> check_utc_timezone!(kind) |> check_usec!(kind) |> DateTime.to_iso8601}
  end
  ...
  def load(:integer, value, _loader), do: load_integer(value)
  ...
  def load(:date, value, _loader), do: load_date(value)
  ...
  defp load_integer(term) when is_integer(term), do: {:ok, term}
  defp load_integer(term) when is_number(term), do: {:ok, floor(term)}
  defp load_integer(_), do: :error

  defp load_date(%Date{} = term), do: {:ok, term}
  defp load_date(term), do: {:ok, Date.from_iso8601!(term)}
  defp load_date(_), do: :error

  defp load_time(%Time{} = time), do: {:ok, truncate_usec(time)}
  defp load_time(term) do
    {:ok, term |> Time.from_iso8601! |> truncate_usec}
  end
  defp load_time(_), do: :error

  defp load_time_usec(%Time{} = time), do: {:ok, pad_usec(time)}
  defp load_time_usec(term) do
    {:ok, term |> Time.from_iso8601! |> pad_usec}
  end
  ...
  defp load_naive_datetime(term) when is_binary(term) do
     {:ok, truncate_usec(NaiveDateTime.from_iso8601!(term))}
  end
  ...
  defp load_naive_datetime_usec(term),
    do: {:ok, term |> NaiveDateTime.from_iso8601! |> pad_usec}
  ...
  defp load_utc_datetime(term) do
    {_, datetime, _} = DateTime.from_iso8601(term)
    {:ok, datetime |> check_utc_timezone!(:utc_datetime) |> truncate_usec()}
  end
  ...
  defp load_utc_datetime_usec(term) do
    {_, datetime, _} = DateTime.from_iso8601(term)
    {:ok, datetime |> check_utc_timezone!(:utc_datetime_usec) |> pad_usec()}
  end
